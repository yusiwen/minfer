...
| **P0.75** | **KV-parallel split attention (decode)** | **~32% decode (shipped 2026-08-03)** | High | attention was measured as the #1 decode bottleneck (~48%, grows with KV): for nt==1 `kernel_gqa_attn_f32` grids only (1, nk)=2 TGs looping the KV sequentially. New `kernel_gqa_attn_partial_f32` (grid (nt,nk,P), per-chunk online-softmax partials) + `kernel_gqa_attn_combine_f32` (grid (nt,nh), merge pass, no shared mem/barriers). nt==1 + f32-cache only (f16 falls back). P=8 default (`MINFER_ATTN_CHUNKS`). Built in `gqa_attn_split_isolation` first (deterministic + cos 1.0 vs scalar at nkv=1/30/33/65/100/240, incl. partial tiles/empty chunks), then A/B byte-identical to classic (`MINFER_NO_SPLIT_ATTN=1`). Median 200-token decode 1.56→1.06 s |
...
| **P1** | **GEMM kernels for non‑Q4_0 quants** | **Q4_K_M ~300→~650, Q5_K_M ~330→~610 t/s prefill (shipped 2026-08-03)** | Large | `kernel_q8_0_mm_f32`/`kernel_q5_0_mm_f32`/`kernel_q5_1_mm_f32` (32-elem blocks, drop-in Q4_0 GEMM + per-quant `dequant_*_16`) + `kernel_q6_k_mm_f32` (256-elem super-block, dequant il = (loop_k%256)/16 + il0). nt≥16 dispatch + 8 KB tg-memory guard. Faithful llama transcriptions (llama block_q6_K has d at the END). Isolation `non_q4_0_gemm_isolation` (deterministic, relerr<5e-3 vs CPU) + A/B byte-identical vs MINFER_GEMM=0. Q4_K/Q5_K still scalar (not in the 0.5B K_M models) |
