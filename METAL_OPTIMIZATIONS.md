...
|:---:|------|--------|--------|-------|
| **P0.5** | **Fused QKV + FFN gate/up matmuls (decode)** | **~24% decode (shipped 2026-08-03)** | Medium | Wq/Wk/Wv and ffn_gate/ffn_up are row-major-concatenated at load (`concat_rows` → `blk.{i}.attn_qkv`/`ffn_gu`) when types + input dim match. nt==1 decode runs ONE matmul/group (od=nqt+2·nkt, 2·nf) into `buf_bqkv`/`buf_bgu`; rope/store/swiglu read sections via `set_buffer` byte offsets. nt==1-only (per-token sections contiguous only then) + exact-length `gpu_abort` guards. Byte-identical to separate path (`MINFER_NO_FUSE_QKV=1` A/B); layout locked in `gemm_isolation.rs::qkv_row_concat_layout`. Median 200-token decode 2.25→1.70 s. FFN half is the bigger win (od=4864, shared input read) |
