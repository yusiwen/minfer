...
fn gqa_attn_isolation() {
    ...
    for &(nt, nkv) in &[(37usize, 37usize), (33, 40), (33, 65), (33, 100)] {
        ...
        let q: Vec<f32> = (0..nt * ne_q)
            .map(|i| ((i as f32) * 1.7).sin() * 3.0)
            .collect();
        let k: Vec<f32> = (0..nkv * nk * hd)
            .map(|i| ((i as f32) * 0.9).cos() * 2.0)
            .collect();
        let v: Vec<f32> = (0..nkv * nk * hd)
            .map(|i| ((i as f32) * 0.4).sin() * 1.5)
            .collect();
        ...
        let qb = unsafe {
            device
                .newBufferWithBytes_length_options(
                    NonNull::new(q.as_ptr() as *const _ as *mut c_void).unwrap(),
                    ((q.len() * 4) as u64) as usize,
        ...
        let kb = unsafe {
            device
                .newBufferWithBytes_length_options(
                    NonNull::new(k.as_ptr() as *const _ as *mut c_void).unwrap(),
                    ((k.len() * 4) as u64) as usize,
                    MTLResourceOptions::StorageModeShared,
                )
                .unwrap()
        };
        let vb = unsafe {
            device
                .newBufferWithBytes_length_options(
                    NonNull::new(v.as_ptr() as *const _ as *mut c_void).unwrap(),
                    ((v.len() * 4) as u64) as usize,
                    MTLResourceOptions::StorageModeShared,
                )
                .unwrap()
        };
        ...
        let k16b = unsafe {
            device
                .newBufferWithBytes_length_options(
                    NonNull::new(k16.as_ptr() as *const _ as *mut c_void).unwrap(),
                    ((k16.len() * 2) as u64) as usize,
                    MTLResourceOptions::StorageModeShared,
                )
                .unwrap()
        };
        let v16b = unsafe {
            device
                .newBufferWithBytes_length_options(
                    NonNull::new(v16.as_ptr() as *const _ as *mut c_void).unwrap(),
                    ((v16.len() * 2) as u64) as usize,
                    MTLResourceOptions::StorageModeShared,
                )
                .unwrap()
        };
        let pb = unsafe {
            device
                .newBufferWithBytes_length_options(
                    NonNull::new(positions.as_ptr() as *const _ as *mut c_void).unwrap(),
                    ((positions.len() * 4) as u64) as usize,
                    MTLResourceOptions::StorageModeShared,
                )
                .unwrap()
        };
        ...
        for (kernel_name, kb_ref, vb_ref) in [
            ...
            let run = |cmdq: &MetalCommandQueue| -> Vec<f32> {
                ...
                unsafe {
                    enc.setBuffer_offset_atIndex(Some(&**(kb_ref)), (0) as usize, (1) as usize)
                };
                unsafe {
                    enc.setBuffer_offset_atIndex(Some(&**(vb_ref)), (0) as usize, (2) as usize)
                };
                ...
                .enumerate()
                {
                    unsafe {
                        enc.setBytes_length_atIndex(
                            NonNull::new(val as *const i32 as *const _ as *mut c_void).unwrap(),
                            (4) as usize,
                            (5 + i as u64) as usize,
                        )
                    };
                ...
                enc.dispatchThreadgroups_threadsPerThreadgroup(
                    MTLSize {
                        width: (nt as u64) as usize,
                        height: (nk as u64) as usize,
                        depth: (1) as usize,
                    },
                    MTLSize {
                        width: (32) as usize,
                        height: (gqa as u64) as usize,
                        depth: (1) as usize,
                    },
                );
                enc.endEncoding();
            ...
            assert!(
                maxdiff_gg == 0.0,
                "{kernel_name} nt={nt} nkv={nkv}: attention non-deterministic"
            );
            assert!(
                cos > 0.999,
                "{kernel_name} nt={nt} nkv={nkv}: attention wrong vs CPU (cos={cos:.6})"
            );
        }
    }
}
...
fn gqa_attn_split_isolation() {
    ...
    let lib = device
        ...
        .unwrap_or_else(|e| panic!("shader compile: {e}"));
    let f_p = lib
        .newFunctionWithName(&*NSString::from_str("kernel_gqa_attn_partial_f32"))
        .unwrap();
    let pl_p = device
        .newComputePipelineStateWithFunction_error(&f_p)
        .unwrap();
    let f_p16 = lib
        .newFunctionWithName(&*NSString::from_str("kernel_gqa_attn_partial_f16"))
        .unwrap();
    let pl_p16 = device
        .newComputePipelineStateWithFunction_error(&f_p16)
        .unwrap();
    let f_c = lib
        .newFunctionWithName(&*NSString::from_str("kernel_gqa_attn_combine_f32"))
        .unwrap();
    let pl_c = device
        .newComputePipelineStateWithFunction_error(&f_c)
        .unwrap();
    ...
    for &(nt, nkv, n_chunks) in &[
        ...
        let run = |cmdq: &MetalCommandQueue,
                   partial_pl: &MetalComputePipelineState,
                   kb_ref: &MetalBuffer,
                   vb_ref: &MetalBuffer|
         -> Vec<f32> {
            let partial = device
                .newBufferWithLength_options(
                    ((nt * nh * n_chunks * (2 + hd) * 4) as u64) as usize,
                    MTLResourceOptions::StorageModeShared,
                )
                .unwrap();
            let ob = device
            ...
            // pass 1: partials
            {
                ...
                unsafe {
                    enc.setBuffer_offset_atIndex(Some(&*partial), (0) as usize, (3) as usize)
                };
                unsafe { enc.setBuffer_offset_atIndex(Some(&*pb), (0) as usize, (4) as usize) };
                for (i, val) in [
                    nh as i32,
                ...
                enc.dispatchThreadgroups_threadsPerThreadgroup(
                    MTLSize {
                        width: (nt as u64) as usize,
                        height: (nk as u64) as usize,
                        depth: (n_chunks as u64) as usize,
                    },
                    MTLSize {
                        width: (32) as usize,
                        height: (gqa as u64) as usize,
            ...
            // pass 2: combine (f32, shared between f32 and f16 caches)
            {
                ...
                unsafe {
                    enc.setBuffer_offset_atIndex(Some(&*partial), (0) as usize, (0) as usize)
                };
                unsafe { enc.setBuffer_offset_atIndex(Some(&*ob), (0) as usize, (1) as usize) };
                for (i, val) in [nh as i32, hd as i32, nt as i32, n_chunks as i32]
                    .iter()
                    .enumerate()
                {
                    unsafe {
        ...
        for (label, pl, kb_ref, vb_ref) in
            [("f32", &pl_p, &kb, &vb), ("f16", &pl_p16, &k16b, &v16b)]
        {
            ...
            assert!(
                maxdiff_gg == 0.0,
                "split[{label}] nt={nt} nkv={nkv} nc={n_chunks}: non-deterministic"
            );
            assert!(
                cos > 0.999,
                "split[{label}] nt={nt} nkv={nkv} nc={n_chunks}: wrong vs CPU (cos={cos:.6})"
            );
        }
    }
}
...
fn gqa_attn_split_timing() {
    ...
    for &(nkv, n_chunks) in &[
        ...
        let partial = device
            .newBufferWithLength_options(
                ((nh * n_chunks * (2 + hd) * 4) as u64) as usize,
                MTLResourceOptions::StorageModeShared,
            )
            .unwrap();
        let ob = device
            .newBufferWithLength_options(
                ((ne_q * 4) as u64) as usize,
                MTLResourceOptions::StorageModeShared,
            )
            .unwrap();
        let shmem = (32 * hd * 2 * 4) as u64;
        ...
        let run = || {
            ...
            unsafe { enc.setBuffer_offset_atIndex(Some(&*ob), (0) as usize, (1) as usize) };
            for (i, val) in [nh as i32, hd as i32, 1, n_chunks as i32]
                .iter()
                .enumerate()
            {
                unsafe {
        ...
        println!(
            "attn timing nkv={nkv} chunks={n_chunks}: {:.4} ms per layer-set (batched x{passes})",
            per * 1e3
        );
    }
}
