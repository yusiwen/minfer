    ...
    for h in 0..nh {
        ...
        for t in 0..nt {
            ...
            for r in 0..nblk * C {
                ...
                for d in 0..hd {
                    o[os + d] += w * kv[nblk * C * nkt + r * nkt + hk * hd + d];
                }
            }
        }
    }
    ...
    fn blk_attn(
        ...
        let pad = self
            ...
            .newBufferWithLength_options(
                ((2 * C as u64 * nkt as u64) * elem) as usize,
                MTLResourceOptions::StorageModeShared,
            )
            .unwrap();
        let ob = self
        ...
        if nkv % C != 0 {
            let f = self
                .lib
                .newFunctionWithName(&*NSString::from_str("kernel_kv_tail_pad"))
                .unwrap();
            let pl = self
                .device
            ...
            for (i, val) in [nkv as i32, nkt as i32, if f16 { 1 } else { 0 }]
                .iter()
                .enumerate()
            {
                unsafe {
            ...
            enc.dispatchThreadgroups_threadsPerThreadgroup(
                MTLSize {
                    width: (nkt as u64) as usize,
                    height: (C as u64) as usize,
                    depth: (1) as usize,
                },
                MTLSize {
                    width: (1) as usize,
        ...
        let (kname, shmem) = if hd == 128 {
            (
                if f16 {
                    "kernel_flash_attn_blk_hd128_f16"
                } else {
                    "kernel_flash_attn_blk_hd128_f32"
                },
                10240u64,
            )
        } else {
            (
                if f16 {
                    "kernel_flash_attn_blk_f16"
                } else {
                    "kernel_flash_attn_blk_f32"
                },
                7168u64,
            )
        };
        ...
        enc.dispatchThreadgroups_threadsPerThreadgroup(
            MTLSize {
                width: ((nt.div_ceil(8)) as u64) as usize,
    ...
    for &(nh, nk, hd) in &[(14usize, 2usize, 64usize), (28, 4, 128)] {
        ...
        for &(nt, nkv) in &[
            ...
            let q: Vec<f32> = (0..nt * ne_q)
                ...
                .collect();
            let k: Vec<f32> = (0..nkv * nkt)
                .map(|i| ((i as f32) * 0.9).cos() * 2.0)
                .collect();
            let v: Vec<f32> = (0..nkv * nkt)
            ...
            let (_, cos_ref, _) = stats(
                &format!("cpu-pad-vs-cpu hd={hd} nt={nt} nkv={nkv}"),
                &ref_pad,
                &ref_out,
            );
            assert!(
                cos_ref > 0.99999,
                "cpu references disagree at hd={hd} nt={nt} nkv={nkv} (cos={cos_ref:.6})"
            );
            ...
            for (label, f16) in [("f32", false), ("f16", true)] {
                ...
                let maxdiff_gg = r1
                    ...
                    .map(|(a, b)| (a - b).abs())
                    .fold(0.0f32, f32::max);
                let (maxdiff, cos, _) = stats(
                    &format!("blk[{label}] hd={hd} nt={nt} nkv={nkv}"),
                    &r1,
                    &ref_pad,
                );
                assert!(
                    maxdiff_gg == 0.0,
                    "blk[{label}] hd={hd} nt={nt} nkv={nkv}: run-to-run differs ({maxdiff_gg:.2e})"
                );
