...
fn gemm_isolation() {
    ...
    let f = lib
        .newFunctionWithName(&*NSString::from_str("kernel_q4_0_mm_f32"))
        .unwrap();
    let pl = device
    ...
    let wb = unsafe {
        device
            .newBufferWithBytes_length_options(
                NonNull::new(weights.as_ptr() as *const _ as *mut c_void).unwrap(),
                ((weights.len()) as u64) as usize,
    ...
    let run = |cmdq: &MetalCommandQueue, nt: usize| -> (Vec<f32>, Vec<f32>) {
        let xb = unsafe {
            device
                .newBufferWithBytes_length_options(
                    NonNull::new(acts.as_ptr() as *const _ as *mut c_void).unwrap(),
                    ((acts.len() * 4) as u64) as usize,
                    MTLResourceOptions::StorageModeShared,
                )
                .unwrap()
        };
        ...
        unsafe {
            enc.setBytes_length_atIndex(
                NonNull::new(mm_p.as_ptr() as *const _ as *mut c_void).unwrap(),
                (12) as usize,
                (3) as usize,
            )
        };
        ...
        enc.dispatchThreadgroups_threadsPerThreadgroup(
            MTLSize {
                width: (((nt + 31) / 32) as u64) as usize,
                height: (((od + 63) / 64) as u64) as usize,
                depth: (1) as usize,
            },
            MTLSize {
                width: (32) as usize,
    ...
    for &nt in &[12usize, 30usize, 32usize, 33usize] {
        ...
        assert!(
            ok_det,
            "nt={nt}: GEMM non-deterministic (maxdiff_gg={maxdiff_gg:.1e})"
        );
        assert!(
            ok_corr,
            "nt={nt}: GEMM wrong vs CPU (maxdiff_ref={maxdiff_ref:.1e} nan={nan1})"
        );
    }
}
...
fn qkv_row_concat_layout() {
    ...
    assert!(
        maxdiff < 1e-6,
        "row-major QKV concat mismatch vs separate matmuls: {maxdiff}"
    );
}
...
fn dq_q6_k(blk: &[u8], e: usize) -> f32 {
    let d = f32::from(half::f16::from_bits(u16::from_le_bytes([
        blk[208], blk[209],
    ])));
...
fn non_q4_0_gemm_isolation() {
    ...
    for &(kname, bbytes, belem, dq) in &quants {
        ...
        let run = || -> Vec<f32> {
            ...
            unsafe {
                enc.setBytes_length_atIndex(
                    NonNull::new(p.as_ptr() as *const _ as *mut c_void).unwrap(),
                    (12) as usize,
                    (3) as usize,
                )
            };
        ...
        println!(
            "{kname}: deterministic={} relerr={:.2e} nan={nan1}",
            maxdiff_gg == 0.0,
            maxdiff_ref / max_abs
        );
        assert!(maxdiff_gg == 0.0, "{kname}: GEMM non-deterministic");
        assert!(
            maxdiff_ref / max_abs < 5e-3 && nan1 == 0,
            "{kname}: GEMM wrong vs CPU (relerr={})",
            maxdiff_ref / max_abs
        );
    }
}
...
fn get_rows_q4_k_isolation() {
    ...
    let f = lib
        .newFunctionWithName(&*NSString::from_str("kernel_get_rows_q4_k"))
        .unwrap();
    let pl = device
        .newComputePipelineStateWithFunction_error(&f)
        .unwrap();
    let wb = unsafe {
        device
            .newBufferWithBytes_length_options(
                NonNull::new(table.as_ptr() as *const _ as *mut c_void).unwrap(),
                (table.len() as u64) as usize,
                MTLResourceOptions::StorageModeShared,
            )
            .unwrap()
    };
    ...
    let ib = unsafe {
        device
            .newBufferWithBytes_length_options(
                NonNull::new(ids_i.as_ptr() as *const _ as *mut c_void).unwrap(),
                ((ids_i.len() * 4) as u64) as usize,
                MTLResourceOptions::StorageModeShared,
            )
            .unwrap()
    };
    ...
    let run = || -> Vec<f32> {
        ...
        unsafe {
            enc.setBytes_length_atIndex(
                NonNull::new(&ne_i as *const i32 as *const _ as *mut c_void).unwrap(),
                (4) as usize,
                (3) as usize,
            )
        };
        unsafe {
            enc.setBytes_length_atIndex(
                NonNull::new(&nt_i as *const i32 as *const _ as *mut c_void).unwrap(),
                (4) as usize,
                (4) as usize,
            )
        };
        ...
        enc.dispatchThreadgroups_threadsPerThreadgroup(
            MTLSize {
                width: (((nt * nsb + 255) / 256) as u64) as usize,
                height: (1) as usize,
                depth: (1) as usize,
            },
            MTLSize {
    ...
    let nan1 = r1.iter().filter(|v| v.is_nan()).count();
    println!(
        "get_rows_q4_k: deterministic={} maxdiff_vs_ref={:.3e} nan={nan1}",
        maxdiff_gg == 0.0,
        maxdiff_ref
    );
    assert!(maxdiff_gg == 0.0, "get_rows_q4_k non-deterministic");
    assert!(
        maxdiff_ref == 0.0 && nan1 == 0,
        "get_rows_q4_k wrong vs CPU (maxdiff={maxdiff_ref})"
    );
}
...
fn get_rows_multi_type_isolation() {
    ...
    let cases = [
        Case {
            kname: "kernel_get_rows_q4_1",
            bbytes: 20,
            is256: false,
            dq: dq_q4_1,
        },
        Case {
            kname: "kernel_get_rows_q5_0",
            bbytes: 22,
            is256: false,
            dq: dq_q5_0,
        },
        Case {
            kname: "kernel_get_rows_q5_1",
            bbytes: 24,
            is256: false,
            dq: dq_q5_1,
        },
        Case {
            kname: "kernel_get_rows_q8_0",
            bbytes: 34,
            is256: false,
            dq: dq_q8_0,
        },
        Case {
            kname: "kernel_get_rows_q6_k",
            bbytes: 210,
            is256: true,
            dq: dq_q6_k,
        },
        Case {
            kname: "kernel_get_rows_q5_k",
            bbytes: 176,
            is256: true,
            dq: dq_q5_k,
        },
    ...
    for case in cases {
        ...
        for (bi, chunk) in table.chunks_mut(case.bbytes).enumerate() {
            ...
            match case.kname {
                "kernel_get_rows_q4_1" => {
                    chunk[0..2].copy_from_slice(
                        &half::f16::from_f32(0.1 + (bi % 5) as f32 * 0.02)
                            .to_bits()
                            .to_le_bytes(),
                    );
                ...
                "kernel_get_rows_q6_k" => {
                    ...
                    for b in 192..208 {
                        chunk[b] = (next() as i8 / 16) as u8;
                    } // scales (small i8)
                    chunk[208..210]
                        .copy_from_slice(&half::f16::from_f32(0.02).to_bits().to_le_bytes());
                }
                _ => {
                    // q5_k
                    chunk[0..2].copy_from_slice(
        ...
        for t in 0..nt {
            ...
            for s in 0..nblk {
                let blk =
                    &table[(row * nblk + s) * case.bbytes..(row * nblk + s + 1) * case.bbytes];
        ...
        let f = lib
            .newFunctionWithName(&*NSString::from_str(case.kname))
            .unwrap();
        let pl = device
            .newComputePipelineStateWithFunction_error(&f)
            .unwrap();
        ...
        enc.dispatchThreadgroups_threadsPerThreadgroup(
            MTLSize {
                width: (((nthreads + 255) / 256) as u64) as usize,
                height: (1) as usize,
                depth: (1) as usize,
            },
            MTLSize {
        ...
        let got =
            unsafe { std::slice::from_raw_parts(ob.contents().as_ptr() as *const f32, nt * ne) };
        ...
        let rel = maxdiff
            / ref_out
                .iter()
                .fold(0.0f32, |a, &v| a.max(v.abs()).max(1e-6));
        assert!(
            rel < 1e-5,
            "{}: get_rows wrong vs CPU (maxdiff={:.3e} rel={:.3e})",
            case.kname,
            maxdiff,
            rel
        );
        println!(
            "  get_rows {}: bit-close vs CPU (rel {:.2e})",
            case.kname, rel
        );
    }
}
