...
impl Ctx {
    ...
    fn flash_attn(
        ...
        let partial = self
            ...
            .newBufferWithLength_options(
                ((nt * self.nh * n_chunks * (2 + self.hd) * 4) as u64) as usize,
                MTLResourceOptions::StorageModeShared,
            )
            .unwrap();
        let ob = self
            .device
            .newBufferWithLength_options(
                ((nt * self.ne_q * 4) as u64) as usize,
                MTLResourceOptions::StorageModeShared,
            )
            .unwrap();
        ...
        enc.dispatchThreadgroups_threadsPerThreadgroup(
            MTLSize {
                ...
                height: (self.nh as u64) as usize,
        ...
        for (i, val) in [self.nh as i32, self.hd as i32, nt as i32, n_chunks as i32]
            .iter()
            .enumerate()
        {
            unsafe {
    ...
    fn split_attn(
        ...
        let kname = if f16 {
            "kernel_gqa_attn_partial_f16"
        } else {
            "kernel_gqa_attn_partial_f32"
        };
        let f = self
            .lib
            .newFunctionWithName(&*NSString::from_str(kname))
        ...
        enc.dispatchThreadgroups_threadsPerThreadgroup(
            MTLSize {
                width: (nt as u64) as usize,
                height: (self.nk as u64) as usize,
                depth: (n_chunks as u64) as usize,
            },
            MTLSize {
                width: (32) as usize,
...
fn run_isolation(ctx: &Ctx) {
    ...
    for &(nt, nkv, n_chunks) in &[
        ...
        let q: Vec<f32> = (0..nt * ctx.ne_q)
            .map(|i| ((i as f32) * 1.7).sin() * 3.0)
            .collect();
        let k: Vec<f32> = (0..nkv * ctx.nk * ctx.hd)
            .map(|i| ((i as f32) * 0.9).cos() * 2.0)
            .collect();
        let v: Vec<f32> = (0..nkv * ctx.nk * ctx.hd)
        ...
        for (label, f16) in [("f32", false), ("f16", true)] {
            ...
            let (maxdiff, cos, _) = stats(
                &format!(
                    "flash[{label}] hd={} nt={nt} nkv={nkv} nc={n_chunks}",
                    ctx.hd
                ),
                &r1,
                &ref_out,
...
fn run_matches_split(ctx: &Ctx) {
    for &(nt, nkv, n_chunks) in &[
        ...
        for (label, f16) in [("f32", false), ("f16", true)] {
            ...
            let (maxdiff, cos, _) = stats(
                &format!(
                    "flash-vs-split[{label}] hd={} nt={nt} nkv={nkv} nc={n_chunks}",
                    ctx.hd
                ),
                &flash,
                &split,
