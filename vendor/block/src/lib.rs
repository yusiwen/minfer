...
// Opaque handle for the Objective-C runtime's `Class` pointer (a block's
// `isa`). Upstream declared `enum Class {}` (uninhabited), which makes the
// extern `static _NSConcreteStackBlock: Class` below uninhabited — rustc
// rejects that as a hard error in future versions (rust-lang/rust#74840).
// A private-field ZST stays opaque but inhabited, so the extern static is
// valid and the symbol address (used as the isa pointer) is unchanged.
#[repr(C)]
