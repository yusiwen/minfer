...
6. [ ] **Prefill attention** — **INVESTIGATED 2026-08-06, low value**: minfer uses the classic `kernel_gqa_attn_f32` for nt>1 (sequential KV-loop, grid (nt,nk), barrier per 32-row tile). Measured attention = 47% of prefill at 510 tokens (140ms/300ms); the prefill gap vs llama grows with length (190→510 tok: 1.18x→1.34x). NOT fixable cheaply: the split attention is SLOWER for prefill (1.27x — tiny 16-row chunks + combine overhead); float4 vectorization won't help (latency-bound, not compute-bound, ~0.12 GOPS at 510 tok). The real fix (llama flash half8x8: grid over KV-tiles, simdgroup matrices) is a ~600-line port for a gap that only appears at 500+ token one-time prefills — LOW VALUE, not pursued. `MINFER_SKIP_ATTN` now also applies during prefill for profiling.
