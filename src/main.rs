...
fn main() {
    ...
    // Stop strings (byte-level, llama.cpp antiprompt): the generated bytes are
    // accumulated in `pending`; output is byte-wise (never lossy), so a
    // multi-byte character split across tokens is passed through intact.
    ...
    while generated.len() < params.n_predict {
        ...
        // Stop-string detection on the byte stream before emitting.
        pending.extend_from_slice(&tokenizer.decode_bytes(&[sampled.token_id]));
        if let Some(cut) = sampler::match_stop_suffix(&pending, &stop_refs) {
            pending.truncate(cut);
            std::io::Write::write_all(&mut std::io::stdout(), &pending).unwrap_or(());
