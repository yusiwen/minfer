...
mod tests {
    ...
    fn matmul_bandwidth_profile() {
        ...
        for &(label, ttype, od, id, n) in cases {
            ...
            unsafe {
                std::slice::from_raw_parts_mut(wb.contents().as_ptr() as *mut u8, wbytes)
                    .fill(0x33);
            }
            let acts = dev
                .newBufferWithLength_options(
    ...
    fn rms_norm_256_correctness() {
        ...
        unsafe {
            std::ptr::copy_nonoverlapping(
                y32.contents().as_ptr() as *const f32,
                y32v.as_mut_ptr(),
                d,
            );
            std::ptr::copy_nonoverlapping(
    ...
    fn prefill_gemm_throughput_profile() {
        ...
        for &(label, ttype, od, id) in cases {
            ...
            let acts = dev
                ...
                .unwrap();
            unsafe {
                std::slice::from_raw_parts_mut(acts.contents().as_ptr() as *mut f32, id * nt)
                    .fill(0.5);
            }
            let out = dev
                .newBufferWithLength_options(
...
mod mmap_align_test {
    ...
    fn nocopy_alignment_probe() {
        ...
        let contents =
            unsafe { std::slice::from_raw_parts(b.contents().as_ptr() as *const u8, 4096) };
