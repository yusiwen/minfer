...
1. **KV positions are data, not structure** — topology never depends on `n_past` (precondition for decode reuse). So is the **allowed attention window**: `attn_span` (E1) carries each query's `[lo, hi)` cell range, resolved by the allocator from `KvCache` ownership + the query's position; `Op::Attn { multi_seq }` marks a node whose batch spans several sequences, and only a backend with `supports_attn_span()` (CPU today; CUDA is E1b, Metal is G5) may take it.
