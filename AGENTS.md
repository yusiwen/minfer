...
- Real-model gates (the `#[ignore]`d set): **run it serially** —
  `cargo test --release --bin minfer -- --ignored --test-threads=1`. Measured 2026-09-24: on a
  CUDA box it is **26 passed / 0 failed** (0.5B config, GB10 sm_121) and on a CPU-only build
  **23 passed / 0 failed**. The numbers this bullet used to carry (**22** on CUDA, **21** on CPU)
  were stale: at `d06f825` they are already **25** and **23** (F7's two tokenizer/chat-template
  reference gates and #123's device-aware packed-cache rework), and F5's real-model gate is the
  26th CUDA-side entry. Parallel, the set is still red on master, because the
  ...
  no longer depends on a quiet box. The Qwen3-0.6B configuration's set is **25/1** (measured
  2026-09-24; it was 24/1 at `d06f825`): `a_slot_snapshot_resumes_the_context_without_re_prefilling`
  fails with *"the file was written with the f32 KV element type, this run uses f16"*, i.e. a
  session cannot encode an f16 KV element type
  ([#130](https://github.com/yusiwen/minfer/issues/130)), a separate pre-existing defect — the only
  failure, and F5's gate passes in that configuration too.
