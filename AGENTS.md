...
2. Each layer owns **two persistent KV regions** (K/V) via `kv_pair(layer)`; they survive rebuilds (allocator lives in `GraphCache`). `kvcache.rs` tracks the owner of every cell (C1), can drop a row range in place (C2), and can compact the arena (C3). A *physical* `kv_rm`/`kv_shift` keeps `cell == pos`, so it stays host-side and no backend gains a kernel; a **compaction renumbers runs**, which needs `Backend::copy_cells` (CPU `copy_within`; CUDA's `kv_move_rows` kernel — downward-only, ascending rows with a barrier between them, no staging buffer, because overlapping device-to-device `cudaMemcpy` is undefined) and Metal refuses it (G5). Because `positions` are cells today, a compaction must also **re-rope the K rows it moves** by `from - to` (C2's `rope_shift_kv` convention) — V is unrotated — so it is `kv_defrag(need, rope)`. Whoever caches a run's `start` — E2's server keeps one per slot — must apply the moves `GraphAllocator::kv_defrag` / `kv_reserve_seq_with_defrag` return.
