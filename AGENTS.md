...
- Real-model gates (the `#[ignore]`d set): one command — `scripts/real_model_gates.sh`
  ...
  **CPU-only** build the *KV-format* reason is gone: since
  ...
  2026-09-25 on this box: **28 passed / 0 failed** serially, and **27 passed / 1 failed** in parallel
  ...
  start-up wave (measured 17.59s batched vs 9.94s serial, and 18.39s vs 9.75s on a rerun); it is the
  ...
  On a CUDA box the set is **31 passed / 0 failed** (0.5B config, GB10 sm_121) and the Qwen3-0.6B
  configuration's set is **31 / 0**; both are still run with `--test-threads=1`. **#123 made the C4
