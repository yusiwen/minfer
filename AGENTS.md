...
- **llama.cpp reference source** (`/Users/yusiwen/git/reading/llama.cpp`, C++ upstream: ggml, quantization kernels, sampler, tokenizer, llama_server, etc.) -> use the `ccc-llamacpp` MCP `search` tool, or CLI: `cd /Users/yusiwen/git/reading/llama.cpp && ccc search "<query>"` (index: 47326 chunks).
- **Structural search** (match code by shape, no index needed): `ccc grep '<pattern>'` -- e.g. `ccc grep 'fn \NAME(\(A*\))' src/` for Rust functions, or `ccc grep '\NAME(\(A*\))'` inside llama.cpp for call sites.
- **Filters**: `--lang rust|cpp|...`, `--path 'src/**'` to narrow results.
- Both MCP servers auto-refresh their indexes (`refresh_index=true` default); if results ever look stale, pass `refresh_index=true` explicitly or run `ccc index` / `ccc search --refresh`.
