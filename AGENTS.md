...
13. **Memory is accounted before it is allocated (E4 S1).** `alloc_in_pool` is fallible: `weights (Backend::weights_bytes) + pooled + this allocation at its size class` is checked against the backend's budget *before* the pool is asked for anything, and the refusal names the numbers (weights / pooled / request / budget, in MiB). The default budget is the backend's own answer — CUDA's current free bytes with a quarter held back (`CudaState::device_memory`); CPU and Metal are unbounded unless `GraphAllocator::set_memory_budget` sets one. `memory_report(backend)` is the accounting surface: `pool_bytes` (the pool's high-water mark — the pools never return memory), `live_bytes`, `peak_live_bytes`, `weights_bytes`, `budget`, `headroom_bytes()`. The class ladder lives in `graph/allocplan.rs` (`class_size`: powers of two to 16 KiB, then 16 KiB steps): the plan and the accounting use it, and **the pools do not round yet** — two shapes in one class still get their own buffer until E4 S2 moves every consumer's length contract to the owning `BufRef` (issue [#55](https://github.com/yusiwen/minfer/issues/55)).
