...
- CUDA test suite on a real GPU: `scripts/cuda_test.sh` (i.e. `cargo test --release --features cuda -- --test-threads=1`; CI has **no** GPU — its CUDA job only compiles the harness — so this is the only way to exercise the device-gated tests; on this box, last full run 2026-09-24 after C4 S2b: **490 passed / 0 failed / 31 ignored**, GB10 sm_121). The real-model gates should be run twice: the cached 0.5B (f32 KV) **and** `MINFER_BATCH_TEST_MODEL=~/.cache/minfer/models/hf/Qwen/Qwen3-0.6B-GGUF/Qwen3-0.6B-Q8_0.gguf` (f16 KV, hd 128 — the only combination that reaches FA prefill and the half-width cell stride; two pre-existing bugs hid behind the f32-only runs). Local GPU runs must rebuild the CLI *with* the feature (`cargo build --release --features cuda`): a plain `cargo test --release` overwrites `target/release/minfer` with a CPU-only build, which silently measures the CPU. `MINFER_DISABLE_CUDA` is presence-checked — `=0` disables CUDA. **Run it serially**: the device state is a process-wide singleton (`CudaState`) whose MMQ memo / captured graph execs / stream state every test shares, so the parallel harness can make one test perturb another's measurement — a parallel-only determinism failure is a harness artifact unless it reproduces serially (issue [#64](https://github.com/yusiwen/minfer/issues/64)). The `#[ignore]`d subset of this command is **31 passed / 0 failed** on the CUDA build (0.5B config, measured 2026-09-24 after C4 S2b).
- Real-model gates (the `#[ignore]`d set): **run it serially** —
  `cargo test --release --bin minfer -- --ignored --test-threads=1`. Measured 2026-09-24 after
  C4 S2b: on a CUDA box it is **31 passed / 0 failed** (0.5B config, GB10 sm_121) and on a CPU-only
  build **28 passed / 0 failed**. (The numbers this bullet used to carry — 26 and 23 — were already
  stale before S2b; every later gate has been added to the count rather than to a footnote.) Parallel,
  the set is still red on master, because the C4 packed-cache gate
  (`a_packed_kv_cache_answers_like_the_f32_one`) sets the **process-wide** KV format for its
  measurement runs, and any test building a graph while it runs then sizes its KV nodes for another
  format (`ensure_kv` refuses the mismatch, correctly); issue
  [#99](https://github.com/yusiwen/minfer/issues/99) is the fix (per-engine format). **#123 made that
  gate device-aware; C4 S2b made it exercise the device**: it now runs **two arms** — the CPU one
  (`--gpu-layers 0`, kept as coverage on every build) and, on a CUDA build with a device, one that
  asserts `device() == Cuda`, so the packed region is measured on the kernels that took the layout
  tag. Each arm asserts its own backend, so a silent CPU fallback fails loudly instead of reporting
  a CPU number as a device one, and a `Drop` guard restores the process-wide KV format even when the
  gate panics. The map-window timing gate
  ...
  Qwen3-0.6B configuration's set is **30 passed / 1 failed** (measured 2026-09-24 after C4 S2b):
  `a_slot_snapshot_resumes_the_context_without_re_prefilling` fails with *"the file was written with
  the f32 KV element type, this run uses f16"*, i.e. a session cannot encode an f16 KV element type
  ([#130](https://github.com/yusiwen/minfer/issues/130)) — a separate pre-existing defect and the
  only failure. A **Q8_0** session does round-trip: `a_session_resumed_from_disk_continues_bitwise`
  with `MINFER_CACHE_TYPE=q8_0` prints `live KV format: q8_0`, saves 24 layers / 256 cells /
  1 696 464 B (vs 6 316 748 B under f32) and continues bitwise (max |Δlogit| = 0), because the
  container's `FLAG_PACKED` bit encodes it.
