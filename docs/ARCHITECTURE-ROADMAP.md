...
| Capability | Status |
|---|---|
| Sequence ids / per-sequence views of one cache | ✔ **C1** per-cell `owner` (`SeqId`); **E1/E2** made several views of one cache real (reservations + explicit `attn_span`, device-aware batching default via E6). One cell still belongs to exactly one sequence — **C8 (scheduled)** turns `owner` into a set/refcount so a prefix can be shared |
| Partial removal / keep / copy between sequences | ◐ **C2**: physical removal of a row range; **compaction inside one arena is C3 (done)**, while *sharing* one cell range across sequences still needs `owner[cell]` to become a set/refcount — **now C8, scheduled right after C7**, with a copy-on-write store rule and a bitwise gate |
...
| Elastic per-slot KV partition (a long request vs `n_slots`) | ✗ **C7 — scheduled next, top priority**: `BatchEngine::new` fixes `cap = n_ctx_total / n_slots` at construction, so a single request is refused once `prompt + generation > cap` even with every other slot idle (`prompt of N tokens exceeds slot context of M`). The arena is sized for the total; only the partition is fixed. C6 is what makes growing a run cheap (a moved row keeps its position, so no other slot's logits change and nothing re-ropes) |
...
| # | Item | Refs | Effort |
|---|---|---|---|
| 1 | **KV cache → sequence-addressable cell store** (cells + seq-id sets; host-resolved `(layer, seq)` → cell indices; explicit per-query mask passed to attention). Prerequisite for everything in P0. — **C1/C2/C3 landed** (cell store, physical removal/shift, compaction) and **C6 landed 2026-09-20** (`001b8cc`: sequence-relative `positions` + an allocator-resolved `cells` input, so compaction is bit-identical). Remaining, **both scheduled**: **C7** (dynamic runs — a single request may use the whole arena; today `cap = n_ctx/n_slots` is fixed at engine construction) and **C8** (`owner` → set/refcount, so a prefix's cells can be shared across sequences, with a copy-on-write store rule). C7 first (user-visible on any multi-slot server), C8 second (only a sharing workload needs it) | §2.4 | XL |
...
| 25 | **Metrics/observability**: `/metrics`, KV occupancy, queue depth, per-op timing under a flag, graceful drain. — **ticketed as F8** (the only member of the A-era batch that had no ticket) | §2.8 | M |
