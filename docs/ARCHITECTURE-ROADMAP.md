...
| Position vs cell | ✔ **C6 (S1–S3 landed 2026-09-19)**: sequence-relative `positions` + an allocator-resolved `cells` input, so a cell move changes no rotation and compaction is bit-identical. The CUDA fused decode QKV family (`Op::FusedQKV`, `Op::QkvBiasRopeStore`) takes `cells` too, so the build-time gate that kept it on the unfused chain under `explicit_span` is now `(cuda_on \|\| !explicit_span)` — Metal keeps it (no explicit-span attention, G5). A request placed in a non-zero-start slot exposed and fixed a server position bug (`submit_on` still added the run start); remaining: S4 docs closure + the PR |
