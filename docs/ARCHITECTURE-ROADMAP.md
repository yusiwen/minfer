...
| Elastic per-slot KV partition (a long request vs `n_slots`) | ◐ **C7 increment 1 (2026-09-20)**: the engine now sizes a slot from the request (`prompt + max_tokens + 1`, clamped to the arena) and, when that exceeds the startup share, reclaims the **idle** slots above it, re-reserves its run in place and re-owns its written prefix — the HTTP bound moved from a slot's share to the whole arena. GB10: `--n-slots 4 --n-ctx 8192` serves a 2054-token prompt (2048 → 2071 cells, three idle slots released) with a continuation byte-identical to `--n-slots 1`. **C7b** remains: a *busy* run above the slot still blocks growth, because `Backend::copy_cells`/CUDA's `kv_move_rows` are downward-only |
...
| # | Item | Refs | Effort |
|---|---|---|---|
| 1 | **KV cache → sequence-addressable cell store** (cells + seq-id sets; host-resolved `(layer, seq)` → cell indices; explicit per-query mask passed to attention). Prerequisite for everything in P0. — **C1/C2/C3 landed** (cell store, physical removal/shift, compaction), **C6 landed 2026-09-20** (`001b8cc`: sequence-relative `positions` + an allocator-resolved `cells` input, so compaction is bit-identical) and **C7 increment 1 landed 2026-09-20** (elastic partition: a long request reclaims idle capacity from the slots above it, and the continuation is byte-identical to a one-slot engine). Remaining: **C7b** (upward row moves, so a busy neighbour cannot block growth) and **C8** (`owner` → set/refcount, so a prefix's cells can be shared, with a copy-on-write store rule) | §2.4 | XL |
