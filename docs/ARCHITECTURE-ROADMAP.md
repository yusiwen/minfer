...
| Quantized KV | ◐ **C4 S1 + S2 (2026-09-23, CPU only)**: `MINFER_CACHE_TYPE=q8_0` stores packed Q8_0 cells — 3.76× smaller regions, measured on both cached models. S2 reads the packed blocks directly (Q8_0 × Q8_0 K dot, V out of the cell: 1.16×/1.31× over S1's dequantizing read at ctx 512/2048) and makes a physical `kv_rm`/`kv_shift` work on a packed region; CUDA/Metal and an unknown value are refused loudly, and their kernels are [#87](https://github.com/yusiwen/minfer/issues/87) · [#42](https://github.com/yusiwen/minfer/issues/42) |
