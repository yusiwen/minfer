...
| 9 | **A sequence's logits' tail depends on its absolute arena offset — measured, attributed, not a bug** (found 2026-09-19 while gating C3; closed the same day by measurement). Same prompt at cell 0 vs cell 8, same relative window, both runs explicit: **2.6% relative** (0.43 absolute) in the **max |Δ| over the whole vocabulary**, with the **greedy token unchanged** and the run deterministic. Attribution, in three probes that are now tests: (1) **the ops are exact** — `the_minimal_attention_graph_is_offset_invariant_to_rounding` runs a hand-built `q`/`k`/`v` → rope → store → attn graph at cells 0, 1 and 8 over five shapes including the model's `(nh=14, nk=2, hd=64)`, and every delta is ≤ 1.2e-7 **and equal for a 1-cell and an 8-cell offset** (so nothing scales with the offset: it is the rotation's own rounding, not a systematic error); (2) **the arena layout is irrelevant** — the same subject cells with the other reservation split in two is bit-identical in every layer's V; (3) **nothing else consumes `positions`** — the model's graph has exactly four such nodes per layer (two ropes, the store, attention; 24×4 = 96), so no position-dependent op hides outside the chain. A layer bisect puts the entry point at **layer 0's attention output** (layer 0's V is bit-identical, layer 1's differs by 3.6e-3), and since the same op at the same shape and window is exact in isolation, the remaining explanation looked like a 1e-7-class input difference **amplified by depth**. **That explanation is refuted (same day, measured):** nudging one element of layer 0's K by 1e-5 — the magnitude of the rotation discrepancy the offset itself produces — leaves the decode-step logits **bitwise unchanged** (0.0) while the offset still moves them by 0.43 (`a_tiny_kv_perturbation_does_not_explain_the_cell_offset_effect`). The probe is not a silent no-op: the **same element nudged by +1.0 moves the logits by 0.359**, i.e. the tail's sensitivity to one K element is ~0.36 per unit and **zero** at 1e-5 — so the offset's 1.5e-5 K discrepancy cannot be its cause, and the offset effect is **structural**, at the O(1) scale. The attribution therefore stays **open**, with two classes eliminated (the ops, the rounding-amplification story) and one method note worth keeping: **a zero from a perturbation probe means nothing without a loud control**. Remaining suspect: the two runs' *query* rope (O(1)-different by construction, and mathematically cancelling), which needs the model's own intermediate buffers **read safely**. The direct attempt failed and taught why: **adding a node to `graph.outputs` does not extend its liveness**, so a rope/attention buffer read after `execute` is whatever the allocator reused it for — the q/k "misalignment" (rel ~1–2) and the suspiciously tiny attention delta it produced were both artefacts of recycled memory. The **persistent KV regions** are the exception (never recycled) and are therefore a test's only safe model-level observation points: measured that way, `the_offset_divergence_appears_between_layer_0_and_layer_1_kv` pins **layer 0's V bit-identical, layer 1's differing by 3.6e-3, layer 2's by 1.2e-2**, so the entry point is at or before layer 0's attention output. Localising it *inside* layer 0 needs a per-node execution path (the CUDA tests' `exec_ids`) or a liveness-aware capture on CPU — that is the next step, and it is recorded rather than faked by reading recycled memory. | measurement | Consequence for C3: the compaction's acceptance is **byte-exact K/V** (`V` verbatim, `K` exactly `rope_shift_kv(old, delta)`) plus **a surviving greedy token**, with the logits' tail in the named amplified-rounding class — standing rule 3's tolerance-class route, with the cause measured rather than assumed. Remaining confirmation (cheap, not yet run): measure the amplification directly by perturbing one K row by ~1e-7 through a public KV write and comparing the logits delta with the 0-vs-8 one. |
