...
| 9 | **A sequence's logits depend on its absolute arena offset** (found 2026-09-19 while gating C3; **narrowed the same day**). Same prompt, same relative window, no compaction: cell 8 vs cell 0 differs by **2.6% relative** (0.43 absolute) on the 0.5B. `offset_sensitivity_is_narrowed_to_multi_query_attention` asserts the parts that hold and prints the rest: the forward is **deterministic**; **one** query token is offset-invariant (a single key makes the softmax weight 1 and V is unrotated, so the scores cannot matter); from **two** tokens on it diverges (0.379 at nt=2). Layer 0's stored V rows are **bit-identical**, the stored K is the cell-rotated K (aligning cell 8 back onto cell 0 with `rope_shift_kv(delta = 8)` matches to ~1e-5), the CPU attention reads the explicit `attn_span` (an unfilled span is an error, so a silent `[0, pos)` fallback is ruled out), and Q/K carry the same freq table (only `n_head` differs, and the table depends on `hd`, not `n_head`). Both runs are forced onto the **same instantiation** (a second reservation makes the control explicit too), so the delta is not a causal-vs-explicit artefact; a CUDA build shows **0 at nt=2** but **0.201 at nt=5**, so it is not CPU-only either. The leak is therefore in the **multi-query QK path**, where a uniform RoPE shift cancels exactly. One pitfall found while probing: reading KV rows directly is only meaningful where the store is **f32** — on a CUDA build the process-wide KV dtype can be f16 (set by whichever model loaded first), and the raw read is then bit patterns, not values (it looks like nondeterminism: 146 between two identical runs). Consequences: (a) C3 cannot claim bit-identical *logits* — it verifies byte-exact K/V plus a surviving greedy token; (b) anything that moves or reuses a slot's cells (cross-slot prefix reuse, dynamic runs) inherits it; (c) it is the floor of the compaction test's delta, and why the server's serial-vs-batched runs had to reuse the same slot layout to come out bitwise. | correctness | Next experiment: the same comparison on a **minimal hand-built graph** (`q`/`k`/`v` inputs → rope → store → attn), which takes the model out of the equation. A cheaper-looking discriminator was tried and is **degenerate — do not repeat it**: giving every token the *same* position removes the relative rotation but also makes them write one cell, so a single key makes the attention output `V` and the comparison trivial (it returns 0 and proves nothing) (`q`/`k`/`v` inputs → rope → store → attn), which takes the model out of the equation; if the leak survives that, it is in the ops, and the **logical-positions** change (sequence-relative `positions` + an allocator-resolved `cells` input) is the fix that removes both it and the C3 re-rope. Reproduce with `offset_sensitivity_is_narrowed_to_multi_query_attention -- --nocapture`. |
